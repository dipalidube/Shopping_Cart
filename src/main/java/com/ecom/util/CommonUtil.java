...
public class CommonUtil {
	...
	public  Boolean sendMail(String url, String reciepentEmail) throws UnsupportedEncodingException, MessagingException {
		...
		helper.setFrom("dipalidube2001@gmail.com", "Shopping Cart");
	 ...
	 * msg=msg.replace("[[orderStatus]]",status); msg=msg.replace("[[productName]]",
	 * order.getProduct().getTitle()); msg=msg.replace("[[category]]",
	 * order.getProduct().getCategory()); msg=msg.replace("[[quantity]]",
	 * order.getQuantity().toString()); msg=msg.replace("[[price]]",
	 * order.getPrice().toString()); msg=msg.replace("[[paymentType]]",
	 * order.getPaymentType());
	 * 
	 * helper.setSubject("Product Order Status"); helper.setText(msg, true);
	 * mailSender.send(message); return true; }
	 * 
	 * public UserDtls getLoggedInUserDetails(Principal p) { String email =
